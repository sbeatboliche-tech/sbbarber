...
export default async function handler(req, res) {
    ...
    const { items, buyer, orderId, total } = req.body;
    ...
    try {
        ...
        await transporter.sendMail({
            ...
            html: sellerHtml(buyer.name, buyer.email, buyer.phone, itemsSummary, total, orderId)
        });
        ...
        if (buyer.email) {
            await transporter.sendMail({
                ...
                html: buyerHtml(buyer.name, itemsSummary, total, orderId)
            });
        }
...
<div style="max-width:480px;margin:0 auto;background:#fff;border-radius:16px;padding:32px;box-shadow:0 4px 24px rgba(0,0,0,.08);">
  ...
  <div style="background:#f8fafc;border-radius:12px;padding:20px;margin-bottom:16px;border:1px solid #e2e8f0;">
    ...
    <p style="color:#0f172a;font-size:17px;font-weight:800;margin:0;">Total: $${Number(total).toLocaleString('es-AR')}</p>
  </div>
