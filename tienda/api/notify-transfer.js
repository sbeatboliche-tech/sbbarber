...
export default async function handler(req, res) {
    ...
    const { items, buyer, orderId, total, discount } = req.body;
    ...
    try {
        ...
        await transporter.sendMail({
            ...
            html: sellerHtml(buyer.name, buyer.email, buyer.phone, itemsSummary, total, discount||0, orderId)
        });
        ...
        if (buyer.email) {
            await transporter.sendMail({
                ...
                html: buyerHtml(buyer.name, itemsSummary, total, discount||0, orderId)
            });
        }
...
function sellerHtml(name, email, phone, items, total, discount, orderId) {
...
<div style="max-width:480px;margin:0 auto;background:#0d1320;border-radius:16px;padding:32px;border:1px solid rgba(255,255,255,0.08);">
  ...
  <div style="background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.2);border-radius:10px;padding:12px;margin-top:16px;">
    <p style="color:#fbbf24;font-size:12px;margin:0;">Esperá la transferencia al alias <strong>sbeat.ar</strong> antes de enviar el producto.</p>
  </div>
  <p style="color:#334155;font-size:11px;margin-top:16px;">Orden: ${orderId}</p>
</div></body></html>`;}
function buyerHtml(name, items, total, discount, orderId) {
